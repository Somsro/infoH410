...
\begin{abstract}
    ...
    In this report, the performance of three different AI techniques on the game 2048 is compared. 
    The three techniques are Expectimax, Deep Q-Learning and an n-tuple network TD learning agent.
    The performance of each approach is evaluated based on the final score reached using 100 runs of each agent.
    Statistical tests are also performed to compare the performance of the agents and to determine if the differences observed are statistically significant.
    Computational cost of each approach is also mentionned to better understand the trade-offs between planning and learning.
...
Thus, Deep Q-Learning which approximates the action value function using a deep neural network is better suited for this task.
Because of its neural network architecture, DQN can handle the high-dimensional state space effectively by learning a compact representation of the state space. \cite{chan_playing_2022}
\textbf{Logic:} Approximate the action value function (Q-value) using a deep neural network to bypass the exhaustive exploration of the state space. \cite{noauthor_reinforcement_nodate}
...
\section{Experimentations}
    ...
    \begin{figure}[h]
        ...
        \caption{Training metrics of the DQN learning agent. The first plot shows the average steps taken per episode, 
        the second plot shows the average score per episode. Each plots also includes mean and standard deviation computed over last runs.}
    ...
    \begin{figure}[h]
        ...
        \caption{Training metrics of the TD learning agent. The first plot shows the average steps taken per episode, 
        the second plot shows the average score per episode. Each plots also includes mean and standard deviation computed over last runs.}
...
\section{Conclusion}
    ...
    however, regarding the training episodes, the DQN agent didn't perform worse compared to the TD learning agent when it was trained for the same number of episodes,
    which suggests that the DQN agent isn't necessarily less sample efficient than the TD learning agent but it is more complex and thus requires more training time to converge.

    While both DQN and TD learning agents need a large amount of training data to reach good performance, 
    the Expectimax agent can reach a good performance without any training, which is a significant advantage in terms of computational cost and time.
    However, the Expectimax agent is much slower than the other two agents during inference, 
    which can be a significant disadvantage in real-time applications or when computational resources are limited.
    ...
    but they are both significantly better than the DQN agent. A huge training time for the DQN agent could improve it's performance but it is not guaranteed and it is not feasible for this project.

    Based on these result, it is possible to conclude that the Expectimax agent is the best choice for this problem if looking for a good performance without the need for training, 
    while the TD learning agent is a good choice if looking for a good performance with a reasonable training time.
